...
public class DataArray {
    ...
    public void merge(Mahasiswa[] workSpace, int lowIndex, 
        ...
        while (lowIndex <= mid && highIndex <= upperBound) {
            if (mhs[lowIndex].getNama().compareTo
                (mhs[highIndex].getNama()) < 0) {
    ...
    public void recQuickSort(int batasKiri, int batasKanan) {
        ...
        //Agustina Mufidatuzzzainiya_210605110144
    public int partitionIt(int batasKiri, int batasKanan, 
            Mahasiswa pivot) {
        int indexKiri = batasKiri - 1;
        int indexKanan = batasKanan + 1;
        while (true) {
            while (indexKiri < batasKanan && mhs[++indexKiri]
                    .getNama().compareTo(pivot.getNama()) < 0)
                ;
            while (indexKanan > batasKiri && mhs[--indexKanan]
                    .getNama().compareTo(pivot.getNama()) > 0)
                ;
